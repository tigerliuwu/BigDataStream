...
public class MRCounterUtil {
	...
	public static boolean validCounterNum(Counters counters, DataProcess dataProcess) {
		...
		long num_delete_rowkey = 0; // 二级删除索引条数
		...
		// 正常二级索引条数 = 有效数据条数 ＊ (定义的正常的二级索引数量 ＊ 2 + 定义的二级索引数量)
		if (num_normal_skey != num_valid_record * (num_skey_normal * 2 + num_skey_other)) {
			valid = false;
			return valid;
		}
