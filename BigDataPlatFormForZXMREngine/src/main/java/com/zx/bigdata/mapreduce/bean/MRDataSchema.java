...
/**
 * 该类实现对数据模型DataSchema的封装，并提供一些接口实现对DataSchema的操作
 * 目前只支持文本的操作，不支持xml
 * @author liuwu
 *
 */
public class MRDataSchema {
	...
	public boolean validDataSchema(String line) throws IOException {
		...
		for (DataItem item : basicSeg.getDataItems()) {
		...
		if (isValid) {
			...
			while (isValid && position < line.length()) {
				
				String subLine = line.substring(position - 1); //报文记录剩下的信息段
	...
	private void clearRecordVal() {
		for (Map.Entry<String, List<Map<String, String>>> entry : recordVal.entrySet()) {
			// 保留报文头中的数据
			if (Segment.CONST_HEADER_KEY.equals(entry.getClass())) {
				continue;
			}
